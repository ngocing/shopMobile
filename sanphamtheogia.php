<?php
	...
	if (isset($_GET['dau']))
	{
		$dau=$_GET['dau']; $dau= mysqli_real_escape_string($conn, $dau);
		if (isset($_GET['cuoi']))
		{
			$cuoi=$_GET['cuoi'];$cuoi= mysqli_real_escape_string($conn, $cuoi);
		}
	}
	$page=0;
	if (isset($_GET['page']))
	...
	if (isset($cuoi))
		$query = mysqli_query($conn,"SELECT * FROM sanpham WHERE  (CASE WHEN giakhuyenmai='0' THEN dongia ELSE giakhuyenmai END)>= ".$dau." and (CASE WHEN giakhuyenmai='0' THEN dongia ELSE giakhuyenmai END)<= ".$cuoi."");
	else 
		$query = mysqli_query($conn,"SELECT * FROM sanpham WHERE  (CASE WHEN giakhuyenmai='0' THEN dongia ELSE giakhuyenmai END)>= ".$dau."");
...
<html><!-- InstanceBegin template="/Templates/fontend.dwt.php" codeOutsideHTMLIsLocked="false" -->
    <head>
        ...
        <title>Sản phẩm theo giá | G1Mobile</title>
        ...
        <a id="button"></a>
            ...
            <div class="wrap">
		        ...
		        <div class="cont span_2_of_3">
                    <h2 class="head">Giá từ <?php echo number_format($dau,"0",",",".");?> vnd<?php if(isset($cuoi)) echo " đến ".number_format($cuoi,"0",",",".")." vnd";?></h2>
                                ...
                                <?php
                                    ...
                                    for($page=0;$page<$sotrang;$page++)
                                    {
                                        if ($page==$k)
                                        {
                                            $n = $page+1;
                                            if (isset($cuoi))
                                                echo "<li><a href='sptheogia.php?page=$page&dau=$dau&cuoi=$cuoi' style='color: #7cb022'>$n</a></li>";
                                            else 
                                                echo "<li><a href='sptheogia.php?page=$page&dau=$dau' style='color: #7cb022'>$n</a></li>";
                                        }
                                        else
                                        {
                                            $n = $page+1;
                                            if (isset($cuoi))
                                                echo "<li><a href='sptheogia.php?page=$page&dau=$dau&cuoi=$cuoi'>$n</a></li>";
                                            else 
                                                echo "<li><a href='sptheogia.php?page=$page&dau=$dau'>$n</a></li>";
                                        }
                                    }
                            	?>
                            </ul>
                    ...
                    <?php
					if (isset($cuoi))
						$query = mysqli_query($conn,"SELECT * FROM sanpham WHERE  (CASE WHEN giakhuyenmai='0' THEN dongia ELSE giakhuyenmai END)>= ".$dau." and (CASE WHEN giakhuyenmai='0' THEN dongia ELSE giakhuyenmai END)<= ".$cuoi."  limit $bd, 4");
					else
						$query = mysqli_query($conn,"SELECT * FROM sanpham WHERE  (CASE WHEN giakhuyenmai='0' THEN dongia ELSE giakhuyenmai END)>= ".$dau."  limit $bd, 4");
					while($row = mysqli_fetch_assoc($query))
					{
						//$q = mysqli_query($conn,"select giakhuyenmai from sanpham where masp ='".$row['masp']."'");
