<?php
	...
	if (isset($_GET['keyword']))
		$key = $_GET['keyword'];$key= mysqli_real_escape_string($conn, $key);
	$page=0;
	if (isset($_GET['page']))
		$page=$_GET['page'];$page= mysqli_real_escape_string($conn, $page);
	$query = mysqli_query($conn,"select * from sanpham where tensp LIKE '%".$key."%'");
...
<html><!-- InstanceBegin template="/Templates/fontend.dwt.php" codeOutsideHTMLIsLocked="false" -->
    <head>
        ...
        <title>Kết quả tìm kiếm cho từ khóa <?php echo $key;?> | G1Mobile</title>
        ...
        <a id="button"></a>
            ...
            <div class="wrap">
		        ...
		        <div class="cont span_2_of_3">
                    <h2 class="head">Kết quả tìm kiếm cho từ khóa '<?php echo $key;?>'(<?php echo $num;?>)</h2>
                                   ...
                                   for($page=0;$page<$sotrang;$page++)
                                     {
                                         ...
                                         {
                                            $n = $page+1;
                                            echo "<li><a href='timkiem.php?keyword=$key&page=$page' style='color: #7cb022'> $n </a></li>";
                                         }
                                         else
                                         {
                                            $n = $page+1;
                                            echo "<li><a href='timkiem.php?keyword=$key&page=$page'> $n </a></li>"; 
                                         }
                                      }
                                  ?>
                            </ul>
                    ...
                    <div class="top-box">
                    <?php
					$query=mysqli_query($conn,"select * from sanpham where tensp LIKE '%".$key."%' order by tensp limit ".$bd.", 3");
